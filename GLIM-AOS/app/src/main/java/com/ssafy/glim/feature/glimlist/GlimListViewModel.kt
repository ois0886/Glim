...
internal class GlimListViewModel @Inject constructor(
    ...
    private val mockUploadedGlims = listOf(
        GlimItem(
            id = 4,
    ...
    fun loadGlimList(listType: GlimListType) = intent {
    ...
    fun toggleLike(glimId: Long) = intent {
        val currentList = state.glimList
        val updatedList = currentList.map { glim ->
            if (glim.id == glimId) {
                glim.copy(
        ...
        try {
            ...
            val message = updatedList.find { it.id == glimId }?.let { glim ->
