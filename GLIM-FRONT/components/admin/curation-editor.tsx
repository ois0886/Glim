 ...
 * 
 * @known_issue
 * 1. 타입 중복: CurationItem 타입이 sortable-list-wrapper.tsx와 중복 정의되어 있습니다.
 *    -> 해결책: types/curation.ts 같은 공유 파일을 만들어 타입을 중앙에서 관리해야 합니다.
 * 2. 검색 기능: 도서/글귀 검색 시 모든 결과를 한 번에 가져옵니다. 데이터가 많아지면 성능 저하의 원인이 될 수 있습니다.
 *    -> 해결책: 백엔드 API에 페이지네이션을 도입하고, 프론트엔드에서는 '더 보기' 버튼이나 무한 스크롤을 구현해야 합니다.
 */
"use client";
...
// --- 타입 임포트 ---
import { CurationItem } from "@/lib/types";
...
import { SortableListWrapper } from '@/components/admin/sortable-list-wrapper';
  ...
  useEffect(() => {
    const fetchCurationData = async (id: string) => {
      try {
        ...
        // [수정됨] API 응답 데이터가 null일 수 있는 경우를 대비해 filter로 안전하게 제거
        const formattedItems: CurationItem[] = itemsSource.map((item) => {
          if (!item) return null; // API 응답에 null이 포함된 경우 건너뛰기
          if (item.bookId) {
            return {
