...
// --- 라이브러리 임포트 ---
import { useState, useEffect, useMemo } from "react" // React 훅: 상태 관리 및 사이드 이펙트 처리
...
export function UserManagement() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axiosInstance.get('/api/v1/admin/members');
        
        // response.data가 이미 사용자 배열이므로, .data를 한번만 사용합니다.
    ...
    if (editedUser && originalUser) {
      try {
        ...
        if (editedUser.status !== originalUser.status) {
          ...
          const statusUpdateResponse = await axiosInstance.patch(`/api/v1/admin/members/${editedUser.memberId}/status`);
  ...
  const handleStatusChangeClick = async (userId: number) => {
    if (window.confirm("이 사용자를 삭제하시겠습니까?")) { // 사용자에게 확인 메시지 표시
        try {
            ...
            const response = await axiosInstance.patch(`/api/v1/admin/members/${userId}/status`);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          {Array.isArray(currentUsers) && currentUsers.every(item => typeof item === 'object' && item !== null) ? (
            <UserTable
              users={currentUsers}
