      ...
      { (curation.bookCount > 0 || curation.quoteCount > 0) && Array.isArray(curation.contents) && (
        <div className="px-5 pt-3 pb-4 border-t">
          <div className="space-y-3">
            {curation.contents.slice(0, 3).map((item) => { // 미리보기는 3개까지만
              if (!item) return null;
              if (item.bookId && item.quoteId === null) {
                return (
                  <div key={`book-${item.bookId}`} className="flex items-center space-x-3">
                    <img
                      src={item.bookCoverUrl || '/placeholder.svg'}
                      alt={item.bookTitle}
                      className="h-16 w-12 rounded-sm object-cover shadow-md flex-shrink-0"
                    />
                    <div className="min-w-0">
                      <p className="text-sm font-semibold leading-tight truncate">{item.bookTitle}</p>
              ...
              if (item.quoteId) {
                 return (
                  <div key={`quote-${item.quoteId}`} className="flex items-center space-x-3">
                     <div className="w-12 h-16 flex items-center justify-center bg-gray-100 rounded-sm flex-shrink-0">
                        <Quote className="h-6 w-6 text-gray-400" />
                     </div>
                     <div className="min-w-0">
                       <p className="text-sm font-semibold leading-tight italic truncate">"{item.content}"</p>
                       <p className="text-xs text-gray-500 truncate">{item.bookTitle} ({item.author})</p>
                     </div>
                   </div>
                 );
              }
             ...
             {curation.contents.length > 3 && <p className="text-xs text-center text-muted-foreground">...외 {curation.contents.length - 3}개</p>}
          </div>
        </div>
      )}
...
export function CurationList({ onNewCuration, onEditCuration }: CurationListProps) {
  ...
  const confirmDeleteCuration = async () => {
    ...
    try {
      ...
      const errorMsg = "삭제 중 서버 오류가 발생했습니다.";
      toast({
        title: "❌ 삭제 실패",
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      setCurations(newOrder); // Optimistic Update
      ...
      try {
        ...
        setCurations(originalCurations); // Revert on error
      }
    }
  };
      ...
      <Card>
        ...
        <CardContent>
          <div className="mb-4 flex justify-end">
            <Button onClick={onNewCuration} variant="default">
      ...
      <AlertDialog open={curationToDelete !== null} onOpenChange={(open) => !open && setCurationToDelete(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={confirmDeleteCuration} className="bg-red-600 hover:bg-red-700">삭제</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
